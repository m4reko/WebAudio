const timeMargin = 300; // Within this margin the time value is considered correct
...
function compare(lastSequence, storedSequence) {
  ...
  const lengthRatio = lastSequence[lastSequence.length - 1].time / storedSequence[storedSequence.length - 1].time;
